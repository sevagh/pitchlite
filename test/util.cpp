...
std::vector<float> test_util::sinewave(size_t size, float frequency,
                                       int sample_rate)
{
    ...
    std::vector<float> tone_single_channel(_tone_single_channel,
                                           _tone_single_channel + size / 2);

    return tone_single_channel;
}

std::vector<float> test_util::vec_from_file(std::string path)
{
