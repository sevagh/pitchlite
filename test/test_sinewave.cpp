...
TEST_P(MpmSinewaveTest, GetFreqManualAlloc)
{
	...
	pitchlite::Mpm ma(data.size());
	double pitch = ma.pitch(data.data(), 48000);
...
TEST_P(YinSinewaveTest, GetFreqManualAlloc)
{
	double freq = GetParam();
	auto data = test_util::sinewave(8092, freq, 48000);
	pitchlite::Yin ya(data.size());
	double pitch = ya.pitch(data.data(), 48000);
	EXPECT_NEAR(freq, pitch, 0.01 * freq);
}
...
TEST(MpmSinewaveTestManualAlloc, OneAllocMultipleFreq)
{
	...
	pitchlite::Mpm ma(data1.size());

	double pitch1 = ma.pitch(data1.data(), 48000);
	double pitch2 = ma.pitch(data2.data(), 48000);
	double pitch3 = ma.pitch(data3.data(), 48000);
...
TEST(YinSinewaveTestManualAlloc, OneAllocMultipleFreq)
{
	...
	pitchlite::Yin ya(data1.size());

	double pitch1 = ya.pitch(data1.data(), 48000);
	double pitch2 = ya.pitch(data2.data(), 48000);
	double pitch3 = ya.pitch(data3.data(), 48000);
